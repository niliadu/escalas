        ...
        <div class="row">
            <div class="tabbable" id="tabs-trocas">
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="panel-lancadas"></div>
                    <div class="tab-pane " id="panel-autorizadas"></div>
...
///////////////////////////
    ...
    function carregarTrocasLancadas() {
        if (definitiva) {
            ...
            ajax.sync.html({
                dados: {tipo: tipo, grupo: grupo, item: 'lancadas'},
                endereco: 'carregar.php',
                sucesso: function (dados) {
    ...
    function autorizarTroca(trocaID, obrigatorio) {
        ajax.sync.html({
            dados: {trocaID: trocaID, obrigatorio: obrigatorio, novoStatus: 2, item: 'justificativa'},
            endereco: 'carregar.php',
            sucesso: function (dados) {
                bootbox.dialog({
                    title: "Justificativa",
    ...
    function atualizarTroca(obrigatorio, trocaID, novoStatus) {
        ...
        autReij = novoStatus == 2 ? "autorização" : 'rejeição';

        tipo = tipo = parseInt($("#tipo_escala").val());
        grupo = parseInt($("#ano-mes-picker").attr('grupo'));
        if (tipo == 2) {
            obrigatorio = false;
            novoStatus = 3;
            ...
            ajax.async.obj({
                ...
                sucesso: function (dados) {
                    ...
                    if (tipo == 2) {
                        ...
                        if (dados[1].id > 0) {
                            idPanel = $('.tab-pane.active').attr('id');
                            idPanel == 'panel-lancadas' ? carregarTrocasLancadas() : carregarTrocasAutorizadas();
                        } else {
